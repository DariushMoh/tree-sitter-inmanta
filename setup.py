...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_inmantazed", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_inmantazed": ["*.pyi", "py.typed"],
        "tree_sitter_inmantazed.queries": ["*.scm"],
    },
    ext_package="tree_sitter_inmantazed",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_inmantazed/binding.c",
                "src/parser.c",
            ],
            define_macros=[
